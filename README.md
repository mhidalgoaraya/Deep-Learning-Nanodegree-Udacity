...
1. create an environment `deep_learning_udacity` with the help of [conda],
   ```
   conda env create -f environment.yaml
   ```
2. activate the new environment with
   ```
   conda activate deep_learning_udacity
   ```
3. install `deep_learning_udacity` with:
   ```
...
2. Create concrete dependencies as `environment.lock.yaml` for the exact reproduction of your
   environment with:
   ```
   conda env export -n deep_learning_udacity -f environment.lock.yaml
   ```
...
├── src
│   └── deep_learning_udacity <- Actual Python package where the main functionality goes.
